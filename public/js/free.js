function onJson1(json) {
  ...
  for(let i=0; i<results ; i+=1){
      ...
      let pref;
      fetch("/home/id/" + result.id).then(onResponse1).then(json => {
        pref = json.preferito;
...
function search(event){ 
  ...
  fetch("/home/" + libro_title).then(onResponse1).then(onJson1);
}
...
const close_m = document.querySelector('.close_menu');
const open_m = document.querySelector('.open_menu');

function Hamburger() {
  if(mobile.classList.contains("show")){ 
    mobile.classList.remove("show");
    close_m.style.display = "none";
    open_m.style.display = "block";
  }else{
    mobile.classList.add("show");
    close_m.style.display = "block";
    open_m.style.display = "none";
  }
}

H.addEventListener('click', Hamburger);
